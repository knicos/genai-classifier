...
export class PoseNet {
    ...
    async estimateMultiplePoses(
        ...
        const allTensorBuffers = await toTensorBuffers3D([heatmapScores, offsets, displacementFwd, displacementBwd]);
        ...
        const poses = decodeMultiplePoses(
            ...
            outputStride,
            configWithDefaults.maxDetections || 5,
...
async function loadMobileNet(config: ModelConfig): Promise<PoseNet> {
    ...
    const url = mobileNetCheckpoint(outputStride, multiplier || 1, quantBytes || 2);
    const graphModel = await tfconv.loadGraphModel(config.modelUrl || url);
    const mobilenet = new MobileNet(graphModel, outputStride);

    const validInputResolution = getValidInputResolutionDimensions(config.inputResolution, mobilenet.outputStride);
...
async function loadResNet(config: ModelConfig): Promise<PoseNet> {
    ...
    const url = resNet50Checkpoint(outputStride, quantBytes || 2);
    const graphModel = await tfconv.loadGraphModel(config.modelUrl || url);
    const resnet = new ResNet(graphModel, outputStride);
    const validInputResolution = getValidInputResolutionDimensions(config.inputResolution, resnet.outputStride);
 ...
 */
export async function load(config: ModelConfig = MOBILENET_V1_CONFIG): Promise<PoseNet> {
