...
export function create(
    ...
    tf.util.assert(
        (customModelArtifactsOrURL == null && customMetadataOrURL == null) ||
            (customModelArtifactsOrURL != null && customMetadataOrURL != null),
        () => `customModelURL and customMetadataURL must be both provided or ` + `both not provided.`
    );
...
export {
    ...
    Dataset,
    type GetDataConfig as GetSpectrogramsAsTensorsConfig,
    getMaxIntensityFrameIndex,
    spectrogram2IntensityCurve,
    type SpectrogramAndTargetsTfDataset,
} from './dataset';
export type {
