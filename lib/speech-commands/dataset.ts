...
export class Dataset {
    ...
    addExample(example: Example): string {
        ...
        tf.util.assert(
            ...
            () => `Expected label to be a non-empty string, ` + `but got ${JSON.stringify(example.label)}`
        );
        const uid = getUID();
    ...
    getExamples(label: string): { uid: string; example: Example }[] {
        ...
        const output: { uid: string; example: Example }[] = [];
    ...
    getData(
        ...
        if (label != null) {
            tf.util.assert(
                ...
                () => `Label ${label} is not in the vocabulary ` + `(${JSON.stringify(vocab)})`
            );
        } else {
        ...
        let hopFrames: number | undefined;
        if (sortedUniqueNumFrames.length === 1) {
            ...
            tf.util.assert(
                numFrames !== undefined && Number.isInteger(numFrames) && numFrames > 0,
                () =>
            ...
            tf.util.assert(
                numFrames !== undefined && numFrames <= sortedUniqueNumFrames[0],
                () =>
            ...
            tf.util.assert(
                hopFrames !== undefined && Number.isInteger(hopFrames) && hopFrames > 0,
                () =>
                    `There are ${sortedUniqueNumFrames.length} unique lengths among ` +
        ...
        return tf.tidy(() => {
            ...
            let uniqueFrameSize: number | undefined;
                ...
                for (const id of ids) {
                    ...
                    if (uniqueFrameSize == null) {
                        ...
                        tf.util.assert(
                            ...
                            () => `Mismatch in frameSize  ` + `(${frameSize} vs ${uniqueFrameSize})`
                        );
                    }
                    ...
                    const windows = getValidWindows(snippetLength, focusIndex ?? 0, numFrames ?? 0, hopFrames ?? 0);
            ...
            if (config.augmentByMixingNoiseRatio != null) {
                this.augmentByMixingNoise(
                    config.getDataset ? xArrays : (xTensors as (Float32Array | tf.Tensor)[]),
                    labelIndices,
            ...
            if (config.getDataset) {
                ...
                const valSplit = config.datasetValidationSplit == null ? 0.15 : config.datasetValidationSplit;
                ...
                const { trainXs, trainYs, valXs, valYs } = balancedTrainValSplitNumArrays(xArrays, yArrays, valSplit);
                ...
                const xTrain = tfd
                    ...
                    .map((x) => tf.tensor3d(x as any, [numFrames ?? 0, uniqueFrameSize ?? 0, 1]));
                ...
                if (shuffle) {
                    // Shuffle the data.
                    const zipped: { x: tf.Tensor3D; y: number }[] = [];
    ...
    private augmentByMixingNoise<T extends tf.Tensor | Float32Array>(
        ...
        const mixedXTensors: (tf.Tensor | Float32Array)[] = [];
        const mixedLabelIndices: number[] = [];
        for (const index of wordExampleIndices) {
            const noiseIndex = noiseExampleIndices[getRandomInteger(0, noiseExampleIndices.length)]; // Randomly sample from the noises, with replacement.
            const signalTensor = isTypedArray ? tf.tensor1d(xs[index] as Float32Array) : (xs[index] as tf.Tensor);
            const noiseTensor = isTypedArray
                ? tf.tensor1d(xs[noiseIndex] as Float32Array)
                : (xs[noiseIndex] as tf.Tensor);
            const mixed: tf.Tensor = tf.tidy(() => normalize(tf.add(signalTensor, tf.mul(noiseTensor, ratio))));
            if (isTypedArray) {
    ...
    setExampleKeyFrameIndex(uid: string, keyFrameIndex: number) {
        ...
        const numFrames = spectrogram.data.length / spectrogram.frameSize;
        tf.util.assert(
            keyFrameIndex >= 0 && keyFrameIndex < numFrames && Number.isInteger(keyFrameIndex),
            () => `Invalid keyFrameIndex: ${keyFrameIndex}. ` + `Must be >= 0, < ${numFrames}, and an integer.`
        );
    ...
    serialize(wordLabels?: string | string[]): ArrayBuffer {
        ...
        return serializedExamples2ArrayBuffer({ manifest, data: concatenateArrayBuffers(buffers) });
    }
}

/** Serialize an `Example`. */
export function serializeExample(example: Example): { spec: ExampleSpec; data: ArrayBuffer } {
    const hasRawAudio = example.rawAudio !== undefined;
    ...
    if (hasRawAudio) {
        ...
        data = concatenateArrayBuffers([data as ArrayBuffer, example.rawAudio?.data.buffer as ArrayBuffer]);
    }
    return { spec, data: data as ArrayBuffer };
}

/** Deserialize an `Example`. */
export function deserializeExample(artifact: { spec: ExampleSpec; data: ArrayBuffer }): Example {
    const spectrogram: SpectrogramData = {
        frameSize: artifact.spec.spectrogramFrameSize,
        data: new Float32Array(
            artifact.data.slice(0, 4 * artifact.spec.spectrogramFrameSize * artifact.spec.spectrogramNumFrames)
        ),
    };
    ...
    if (artifact.spec.rawAudioNumSamples != null) {
        ex.rawAudio = {
            sampleRateHz: artifact.spec.rawAudioSampleRateHz ?? 16000,
            data: new Float32Array(
                artifact.data.slice(4 * artifact.spec.spectrogramFrameSize * artifact.spec.spectrogramNumFrames)
            ),
        };
    }
    return ex;
 ...
 */
function serializedExamples2ArrayBuffer(serialized: SerializedExamples): ArrayBuffer {
    const manifestBuffer = string2ArrayBuffer(JSON.stringify(serialized.manifest));
    ...
    const headerBuffer = concatenateArrayBuffers([descriptorBuffer, version.buffer, manifestLength.buffer]);
...
export function arrayBuffer2SerializedExamples(buffer: ArrayBuffer): SerializedExamples {
    tf.util.assert(buffer != null, () => 'Received null or undefined buffer');
    // Check descriptor.
    let offset = 0;
    const descriptor = arrayBuffer2String(buffer.slice(offset, DATASET_SERIALIZATION_DESCRIPTOR.length));
    tf.util.assert(descriptor === DATASET_SERIALIZATION_DESCRIPTOR, () => `Deserialization error: Invalid descriptor`);
...
export function getValidWindows(
    ...
    tf.util.assert(
        ...
        () => `windowLength (${windowLength}) exceeds snippetLength ` + `(${snippetLength})`
    );
    tf.util.assert(
        focusIndex < snippetLength,
        () => `focusIndex (${focusIndex}) equals or exceeds snippetLength ` + `(${snippetLength})`
    );
 ...
 *   `[numFrames]`.
 */
export function spectrogram2IntensityCurve(spectrogram: SpectrogramData): tf.Tensor {
    return tf.tidy(() => {
        const numFrames = spectrogram.data.length / spectrogram.frameSize;
