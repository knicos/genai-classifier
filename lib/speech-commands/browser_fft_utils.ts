...
export async function loadMetadataJson(url: string): Promise<{ wordLabels: string[] }> {
    ...
    if (url.indexOf(HTTP_SCHEME) === 0 || url.indexOf(HTTPS_SCHEME) === 0) {
        ...
        throw new Error(
            `Loading metadata from file URLs is not supported in the browser. ` +
                `Please use HTTP or HTTPS URLs, or run this code in a Node.js environment.`
        );
    } else {
        throw new Error(
 ...
 */
export function playRawAudio(rawAudio: RawAudioData, onEnded: () => void | Promise<void>): void {
