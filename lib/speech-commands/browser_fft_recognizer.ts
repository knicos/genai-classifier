 ...
 */
export class BrowserFftSpeechCommandRecognizer implements SpeechCommandRecognizer {
    static readonly VALID_VOCABULARY_NAMES: string[] = ['18w', 'directional4w'];
    static readonly DEFAULT_VOCABULARY_NAME = '18w';

    public id = `recognizer-${id++}`;

    readonly MODEL_URL_PREFIX = `https://store.gen-ai.fi/tm/models/speech-commands`;
    ...
    private elementsPerExample: number = 1;
    protected audioDataExtractor!: BrowserFftFeatureExtractor;

    private transferRecognizers: { [name: string]: TransferBrowserFftSpeechCommandRecognizer } = {};
    ...
    constructor(
        vocabulary?: string,
        modelArtifactsOrURL?: tf.io.ModelArtifacts | string,
        ...
        tf.util.assert(
            (modelArtifactsOrURL == null && metadataOrURL == null) ||
                (modelArtifactsOrURL != null && metadataOrURL != null),
            () => `modelURL and metadataURL must be both provided or ` + `both not provided.`
        );
        ...
        if (modelArtifactsOrURL == null) {
            if (vocabulary == null) {
                ...
                tf.util.assert(
                    BrowserFftSpeechCommandRecognizer.VALID_VOCABULARY_NAMES.indexOf(vocabulary) !== -1,
            ...
            if (!metadataOrURL) throw new Error('metadataOrURL must be provided when modelArtifactsOrURL is provided');
     ...
     */
    async listen(callback: RecognizerCallback, config?: StreamingRecognitionConfig): Promise<void> {
        if (this.streaming) {
        ...
        let invokeCallbackOnNoiseAndUnknown =
            config.invokeCallbackOnNoiseAndUnknown == null ? false : config.invokeCallbackOnNoiseAndUnknown;
        if (config.includeEmbedding) {
        ...
        if ((config.suppressionTimeMillis ?? 0) < 0) {
            throw new Error(
                `suppressionTimeMillis is expected to be >= 0, ` + `but got ${config.suppressionTimeMillis}`
            );
        }
        ...
        const spectrogramCallback: SpectrogramCallback = async (x: tf.Tensor) => {
            ...
            let y: tf.Tensor;
            let embedding: tf.Tensor | undefined = undefined;
            if (config.includeEmbedding) {
                await this.ensureModelWithEmbeddingOutputCreated();
                [y, embedding] = this.modelWithEmbeddingOutput.predict(normalizedX) as tf.Tensor[];
            } else {
            ...
            if (maxScore < probabilityThreshold) {
                ...
                let spectrogram: SpectrogramData | undefined = undefined;
        ...
        const suppressionTimeMillis =
            config.suppressionTimeMillis == null ? this.DEFAULT_SUPPRESSION_TIME_MILLIS : config.suppressionTimeMillis;
    ...
    async ensureModelLoaded() {
        ...
        this.parameters.spectrogramDurationMillis = (numFrames ?? 0) * frameDurationMillis;
    }
    ...
    protected async ensureModelWithEmbeddingOutputCreated() {
        ...
        let secondLastDenseLayer: tfl.layers.Layer | undefined = undefined;
        ...
        this.modelWithEmbeddingOutput = tfl.model({
            ...
            outputs: [this.model.outputs[0], secondLastDenseLayer.output as tfl.SymbolicTensor],
        });
    }
    ...
    private async ensureMetadataLoaded() {
        ...
        const metadataJSON =
            typeof this.metadataOrURL === 'string' ? await loadMetadataJson(this.metadataOrURL) : this.metadataOrURL;
    ...
    async recognize(
        ...
        if (input instanceof tf.Tensor) {
            ...
            if (this.elementsPerExample === 1) {
                this.elementsPerExample = 1;
                this.model.inputs[0].shape.slice(1).forEach((dimSize) => (this.elementsPerExample *= dimSize ?? 1));
            }
            ...
            inputTensor = tf.tensor4d(
                input,
                [numExamples].concat(this.nonBatchInputShape) as [number, number, number, number]
            );
        }

        const output: SpeechCommandRecognizerResult = { scores: [] };
        if (config.includeEmbedding) {
            // Optional inclusion of embedding (internal activation).
            await this.ensureModelWithEmbeddingOutputCreated();
            const outAndEmbedding = this.modelWithEmbeddingOutput.predict(inputTensor) as tf.Tensor[];
        ...
        if (config.includeSpectrogram) {
            output.spectrogram = {
                data: (input instanceof tf.Tensor ? await input.data() : input) as Float32Array,
                frameSize: this.nonBatchInputShape[1],
            };
        }
    ...
    private checkInputTensorShape(input: tf.Tensor) {
        ...
        if (!tf.util.arraysEqual(nonBatchedShape, expectedNonBatchShape.filter((dim) => dim != null) as number[])) {
            throw new Error(
 ...
 */
class TransferBrowserFftSpeechCommandRecognizer
    extends BrowserFftSpeechCommandRecognizer
    implements TransferSpeechCommandRecognizer
{
    ...
    constructor(
        ...
        tf.util.assert(
            ...
            () => `The name of a transfer model must be a non-empty string, ` + `but got ${JSON.stringify(name)}`
        );
        this.nonBatchInputShape = this.baseModel.inputs[0].shape.slice(1) as [number, number, number];
        this.words = [];
     ...
     */
    async collectExample(word: string, options?: ExampleCollectionOptions): Promise<SpectrogramData> {
        tf.util.assert(
            !this.streaming,
            () =>
        ...
        if (options.durationSec != null) {
            ...
            const frameDurationSec = (this.parameters.fftSize ?? 0) / (this.parameters.sampleRateHz ?? 1);
    ...
    countExamples(): { [word: string]: number } {
        if (this.dataset.empty()) {
            throw new Error(
                `No examples have been collected for transfer-learning model ` + `named '${this.name}' yet.`
            );
        }
     ...
     */
    getExamples(label: string): { uid: string; example: Example }[] {
    ...
    private collectTransferDataAsTensors(
        ...
        const out = this.dataset.getData(undefined, { numFrames, hopFrames, ...augmentationOptions }) as {
            xs: tf.Tensor4D;
            ys?: tf.Tensor2D;
        };
    ...
    private collectTransferDataAsTfDataset(
        ...
        windowHopRatio = windowHopRatio || DEFAULT_WINDOW_HOP_RATIO;
        const hopFrames = Math.round(windowHopRatio * numFrames);
        return this.dataset.getData(undefined, {
            numFrames,
            hopFrames,
            getDataset: true,
     ...
     */
    async train(config?: TransferLearnConfig): Promise<tfl.History | [tfl.History, tfl.History]> {
        tf.util.assert(
        ...
        if (config?.fineTuningEpochs != null) {
            tf.util.assert(
                config.fineTuningEpochs >= 0 && Number.isInteger(config?.fineTuningEpochs),
                () =>
    ...
    private async trainOnDataset(config?: TransferLearnConfig): Promise<tfl.History | [tfl.History, tfl.History]> {
        tf.util.assert((config?.epochs ?? 0) > 0, () => `Invalid config.epochs`);
    ...
    private async trainOnTensors(config?: TransferLearnConfig): Promise<tfl.History | [tfl.History, tfl.History]> {
        // Prepare the data.
        const windowHopRatio = config?.windowHopRatio || DEFAULT_WINDOW_HOP_RATIO;
        ...
        try {
            ...
            if (config?.fineTuningEpochs != null && config.fineTuningEpochs > 0) {
                ...
                // model.
                if (!valData) throw new Error(`Cannot perform fine-tuning without validation data.`);
                const fineTuningHistory = await this.fineTuningUsingTensors(config, trainXs, trainYs, valData);
                return [history, fineTuningHistory];
            } else {
                return history;
            }
    ...
    private async fineTuningUsingTfDatasets(
        ...
        trainDataset: tfd.Dataset<{}>,
        valDataset: tfd.Dataset<{}>
    ): Promise<tfl.History> {
        ...
        const fineTuningHistory = await this.model.fitDataset(trainDataset, {
            epochs: config?.fineTuningEpochs ?? 1,
    ...
    private async fineTuningUsingTensors(
        ...
        const fineTuningHistory = await this.model.fit(trainXs, trainYs, {
            ...
            validationData: valData,
            batchSize: config?.batchSize,
            callbacks: config?.fineTuningCallback == null ? undefined : [config.fineTuningCallback],
        });
        // Set the trainable attribute of the fine-tuning layer to its
        // previous value.
        this.secondLastBaseDenseLayer.trainable = originalTrainableValue;
    ...
    async evaluate(config: EvaluateConfig): Promise<EvaluateResult> {
        tf.util.assert(
            config.wordProbThresholds != null && config.wordProbThresholds.length > 0,
        ...
        tf.util.assert(
            ...
            () => `Cannot perform evaluation when the first tag is not ` + `${BACKGROUND_NOISE_TAG}`
        );
        ...
        return tf.tidy(() => {
            ...
            // negative class.
            const maxWordProbs = tf.max(tf.slice(probs, [0, 1], [probs.shape[0], (probs.shape[1] ?? 0) - 1]), -1);
            ...
            for (let i = 0; i < config.wordProbThresholds.length; ++i) {
                ...
                if (i > 0) {
                    // Accumulate to AUC.
                    auc +=
                        (Math.abs(rocCurve[i - 1].fpr - rocCurve[i].fpr) * (rocCurve[i - 1].tpr + rocCurve[i].tpr)) / 2;
                }
            }
            return { rocCurve, auc };
        });
    ...
    private createTransferModelFromBaseModel(): void {
        tf.util.assert(
            this.words != null,
            () => `No word example is available for tranfer-learning model of name ` + this.name
        );
        ...
        const truncatedBaseOutput = this.secondLastBaseDenseLayer.output as tfl.SymbolicTensor;
        ...
        const transferOutput = this.transferHead.apply(truncatedBaseOutput) as tfl.SymbolicTensor;
        this.model = tfl.model({ inputs: this.baseModel.inputs, outputs: transferOutput });
    }
     ...
     * @param name
     */
    createTransfer(): TransferBrowserFftSpeechCommandRecognizer {
        throw new Error(
