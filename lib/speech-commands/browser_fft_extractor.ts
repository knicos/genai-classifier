...
export type SpectrogramCallback = (freqData: tf.Tensor, timeData?: tf.Tensor) => Promise<boolean>;
...
export class BrowserFftFeatureExtractor implements FeatureExtractor {
    ...
    constructor(config: BrowserFftFeatureExtractorConfig) {
        ...
        this.includeRawAudio = config.includeRawAudio || false;
    ...
    async start(audioTrackConstraints?: MediaTrackConstraints): Promise<void> {
        ...
        this.audioContext = new this.audioContextConstructor({ sampleRate: this.sampleRateHz }) as AudioContext;
    ...
    private async onAudioFrame() {
        ...
        if (shouldFire) {
            ...
            let timeDataTensor: tf.Tensor | undefined;
            if (this.includeRawAudio) {
...
export function getInputTensorFromFrequencyData(freqData: Float32Array, shape: number[]): tf.Tensor {
